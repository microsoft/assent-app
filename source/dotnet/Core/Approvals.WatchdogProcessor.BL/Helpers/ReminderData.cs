...
namespace Microsoft.CFS.Approvals.WatchdogProcessor.BL.Helpers
{
    ...
    public class ReminderData : IReminderData
    {
            ...
            TableQuery<ApprovalSummaryRow> query = new TableQuery<ApprovalSummaryRow>()
                .Where(
                        TableQuery.CombineFilters(
                                TableQuery.GenerateFilterConditionForDate("NextReminderTime", QueryComparisons.LessThanOrEqual, currentTime),
                                TableOperators.And,
        ...
        public void UpdateReminderInfo(ApprovalTenantInfo approvalTenantInfo, ApprovalSummaryRow summaryToUpdate)
        {
            ...
            ApprovalDetailsEntity approvalDetails = new ApprovalDetailsEntity() { PartitionKey = summaryJson?.ApprovalIdentifier?.GetDocNumber(approvalTenantInfo), RowKey = Constants.SummaryOperationType, ETag = "*", JSONData = summaryRows.ToJson(), TenantID = approvalTenantInfo.TenantId };

            var summaryTableName = _config[ConfigurationKey.ApprovalSummaryAzureTableName.ToString()];
            try
            {
                _tableHelper.InsertOrReplace(summaryTableName, summaryToUpdate);
                _tableHelper.InsertOrReplace(Constants.ApprovalDetailsAzureTableName, approvalDetails);
            ...
            catch (Exception ex)
            {
                ...
                _tableHelper.InsertOrReplace(summaryTableName, changedRow);
            }
        }
        ...
        private ApprovalSummaryRow RetrieveSummaryRow(string partitionKey, string rowKey)
        {
            ...
            {
                _logger.LogError(TrackingEvent.WatchDogReminderRetrieveSummaryRowsFail, ex);
                throw;
            }
        }
