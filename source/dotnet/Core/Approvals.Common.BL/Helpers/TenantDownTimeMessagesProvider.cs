...
public class TenantDownTimeMessagesProvider : ITenantDownTimeMessagesProvider
{
    ...
    {
        var notificationList = _tableHelper.GetTableEntity<TenantDownTimeMessages>(_config[ConfigurationKey.TenantDownTimeMessagesAzureTableName.ToString()]);
    ...
    public bool InsertOrUpdateTenantDowntimeMessage(ApprovalTenantInfoRealTime realtimeTenantInfo, string loggedInAlias)
    {
        try
        {
            string query = "RowKey eq '" + realtimeTenantInfo.DocTypeId + "'";
            var tenantDownTimeMessage = _tableHelper.GetDataCollectionByTableQuery<TenantDownTimeMessages>(_config[ConfigurationKey.TenantDownTimeMessagesAzureTableName.ToString()],
