...
namespace Approvals.SecondaryProcessor.BL
{
    ...
    public class SecondaryReceiver : IApprovalsQueueReceiver
    {
        #region Properties
        ...
        /// <param name="secondaryProcessor"></param>
        public SecondaryReceiver
            (IPerformanceLogger performanceLogger,
            ILogProvider logProvider,
            IConfiguration configuration,
            IBlobStorageHelper blobStorageHelper,
            ISecondaryProcessor secondaryProcessor)
        {
        ...
        public async Task OnMainMessageReceived(ServiceBusReceivedMessage message, string blobId)
        {
            if (!string.IsNullOrWhiteSpace(blobId))
            {
                ...
                {
                    if ((message.ApplicationProperties.ContainsKey("Source") &&
                        message.ApplicationProperties["Source"].ToString().Equals(Constants.ServiceBusSourcePrimary, StringComparison.InvariantCultureIgnoreCase)) || (!message.ApplicationProperties.ContainsKey("Source")))
                    {
                        await Task.Run(() => ProcessMessageOnTaskAndAwait(blobId, message));
                    }
                    else if (message.ApplicationProperties.ContainsKey("Source"))
                    {
                        await _secondaryProcessor.ProcessSecondaryDetailsAsync(new ApprovalRequestExpressionExt(), blobId, TenantInfo, message, false);
                    }
                }
            }
        }
        ...
        private async Task ProcessMessageOnTaskAndAwait(string blobId, ServiceBusReceivedMessage serviceBusMessage)
        {
            ...
            using (var perfTracer = _performanceLogger.StartPerformanceLogger("PerfLog", "SecondaryWorker", string.Format(Constants.PerfLogAction, "Secondary", "Processes the message on a separate thread and calls remaining workflow"), logData))
            {
                try
                {
                    logData.Add(LogDataKey.QueueName, _config[ConfigurationKey.QueueNameSecondary.ToString()]);

                    byte[] message;
                    message = await _blobStorageHelper.DownloadByteArray(Constants.SecondaryMessageContainer, blobId);
                    ...
                    //stream.Position = 0;
                    if (serviceBusMessage.ApplicationProperties.ContainsKey("ContentType") &&
                        serviceBusMessage.ApplicationProperties["ContentType"].ToString().Equals("ApprovalRequestExpression", StringComparison.InvariantCultureIgnoreCase))
                    {
                        ...
                        _logProvider.LogInformation(TrackingEvent.ARXReceivedBySecondaryWorker, logData);
                ...
                {
                    _logProvider.LogError(TrackingEvent.ARXFailedBySecondaryWorker, ex, logData);
                }
                catch (Exception ex)
                {
                    _logProvider.LogError(TrackingEvent.ARXFailedBySecondaryWorker, ex, logData);
        ...
        {
            using (var perfTracer = _performanceLogger.StartPerformanceLogger("PerfLog", "SecondaryWorker", string.Format(Constants.PerfLogAction, TenantInfo.AppName, "Processes received Brokered Message")
                   , new Dictionary<LogDataKey, object>())) // TODO: log message ID here.
            {
                if (numberOfRetries >= 0)
                {
                    ...
                    var failedRequest = await _secondaryProcessor.ProcessSecondaryDetailsAsync(requestExpressions, blobId, TenantInfo, sbMessage, false);
                    ...
                    {
                        if (await _blobStorageHelper.DoesExist(Constants.SecondaryMessageContainer, blobId))
                        {
