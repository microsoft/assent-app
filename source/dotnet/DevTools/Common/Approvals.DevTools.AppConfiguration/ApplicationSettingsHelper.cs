...
namespace Microsoft.CFS.Approvals.DevTools.AppConfiguration;
using System;
        ...
        foreach (var label in labels)
        {
            ...
            foreach (var setting in setingsByLabel)
            {
                if (setting is SecretReferenceConfigurationSetting secretReference)
                {
                    ...
                    // Use the same credential selection for KeyVault SecretClient
                    var secretClient = new SecretClient(identifier.VaultUri, credential);
                    // Always pass CancellationToken.None for GetSecretAsync
                    var secret = secretClient.GetSecretAsync(identifier.Name, identifier.Version, System.Threading.CancellationToken.None).Result;
